...
const localMessageBuffer: Array<{ from_id: string; from_summary: string; from_cwd: string; from_machine: string; text: string; sent_at: string }> = [];
...
mcp.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  switch (name) {
    ...
    case "set_summary": {
      ...
      // Bug 1b: fail LOUD on a malformed call. The broker coalesces a missing summary to ''
      // (defense-in-depth, no 500) — but that silently blanks the caller's summary, so a wrong
      // arg name (e.g. `message` instead of `summary`) would store empty and go unnoticed.
      // Reject a non-(non-empty-string) summary here so caller error surfaces loudly.
      if (typeof summary !== "string" || summary.trim() === "") {
        return {
          content: [{ type: "text" as const, text: 'set_summary requires a non-empty "summary" string argument.' }],
          isError: true,
        };
      }
      mySummary = summary;
    ...
    case "check_messages": {
      ...
      try {
        // First check broker for any messages the poll loop hasn't grabbed yet
        ...
        const lines = messages.map(
          (m) => `From ${m.from_id}${m.from_machine ? ` [${m.from_machine}]` : ""} (${m.sent_at}):\n${m.text}`
        );
        return {
          content: [
            {
...
async function pollAndPushMessages() {
  ...
  try {
    ...
    for (const msg of result.messages) {
      ...
      // Buffer locally so check_messages works even if channel push fails
      localMessageBuffer.push({
        from_id: msg.from_id,
      ...
      if (!CHANNEL_DISABLED) {
        ...
        try {
          ...
          // Push failed — message stays in the local buffer (check_messages will surface it).
          // Never stay silent here: a silent catch hid a weeks-long push outage (an old client
          // binary couldn't handle the experimental claude/channel capability, so every push
          // threw straight into this block and delivery silently degraded to poll-only).
          log(`Channel push FAILED for msg from ${msg.from_id} (${fromMachine}): ${e instanceof Error ? e.message : String(e)} — retained in poll buffer`);
        }
      }

      log(`${pushed ? "Pushed" : "Buffered (no channel push)"} message from ${msg.from_id} (${fromMachine}): ${msg.text.slice(0, 80)}`);
    }
  } catch (e) {
